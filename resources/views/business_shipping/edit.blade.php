<form id="shippingForm" action="{{ route('business-shippings.update', $shipping->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div id="card-image-uploader-modal" data-output-width="400" data-output-height="400" data-aspect-ratio="1"
        ...
        <div class="ciu-box mx-auto position-relative"
            ...
            @endphp
            <img class="ciu-preview" id="imagePreview"
                 src="{{ $imageUrl }}"
                 style="{{ $imageUrl ? 'display:block;' : 'display:none;' }} max-width:100%; max-height:500px;">
            <span class="ciu-placeholder" style="{{ $imageUrl ? 'display:none;' : 'font-size:2rem; color:#888;' }}">+ Image</span>
        </div>
