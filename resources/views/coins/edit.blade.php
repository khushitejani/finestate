<form action="{{ route('coins.update', $coin->id) }}" method="POST" enctype="multipart/form-data" id="coinForm">
    @csrf
    @method('PATCH')

    <div class="mb-3">
        <label class="form-label fw-bold d-block">Coin Image</label>
        ...
        <div class="ciu-box mx-auto position-relative" id="imageBox"
            ...
            <img id="imagePreview" class="ciu-preview" alt="Preview"
                src="{{ isset($coin) && $coin->image ? asset('storage/' . $coin->image) : '' }}"
                style="display: {{ isset($coin) && $coin->image ? 'block' : 'none' }};
                   width:auto; height:auto; max-width:100%; max-height:500px;">

            <span class="ciu-placeholder"
                style="font-size:2rem; color:#888; 
              {{ isset($coin) && $coin->image ? 'display:none;' : '' }}">+
                Image</span>
        </div>
    ...
    <div class="mb-3 mt-3">
        ...
        <input type="text" name="name" class="form-control" value="{{ old('name', $coin->name) }}" required>
        @error('name')
            <small class="text-danger">{{ $message }}</small>
        @enderror
    </div>
