<form action="{{ route('aircraftshops.update', $aircraftShop->id) }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="mb-3">
        ...
        <textarea name="description" rows="4" class="form-control" required>{{ old('description', $aircraftShop->description) }}</textarea>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        function addImageElement(fileObj, src, existing) {
            ...
            const img = $('<img>').attr('src', src).addClass('rounded border thumbnail').css({
                width: '100%',
                height: '100%',
                objectFit: 'cover',
                cursor: 'pointer'
