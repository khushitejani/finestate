    ...
    function updatePreview() {
        ...
        filesArray.forEach((file, index) => {
            ...
            reader.onload = function(e) {
                const wrapper = $('<div>').addClass('position-relative me-2 mb-2').css({width:'100px', height:'80px'}).attr('data-index', index);
                const img = $('<img>').attr('src', e.target.result).addClass('rounded border thumbnail').css({width:'100%', height:'100%', objectFit:'cover', cursor:'pointer'});
                const removeBtn = $('<button>').attr('type','button').addClass('btn btn-sm btn-danger position-absolute top-0 end-0 remove-image').text('×');

                wrapper.append(img).append(removeBtn);
                preview.append(wrapper);
                if (index === 0) {
                    mainWrapper.html(`<img id="mainPreview" src="${e.target.result}" style="width:100%; height:100%; object-fit:cover;">`);
                }
                img.on('click', function() {
                    mainWrapper.html(`<img id="mainPreview" src="${this.src}" style="width:100%; height:100%; object-fit:cover;">`);
                });
